...
class DeepSeekClient:
    ...
    async def enhance_transcript(self, transcript_turns: List[TranscriptTurn]) -> List[EnhancedTranscriptTurn]:
        """使用 DeepSeek 优化转录文本"""
        ...
        try:
            ...
            # 解析响应
            enhanced_turns = self._parse_response(response.choices[0].message.content, transcript_turns)
...
4. 输出 JSON 格式"""
    ...
    def _parse_response(self, response_content: str, original_turns: List[TranscriptTurn]) -> List[EnhancedTranscriptTurn]:
        """解析 LLM 响应"""
        ...
        for i, item in enumerate(enhanced_data):
            if i < len(original_turns):
                ...
                enhanced_text = item.get("text", original_turns[i].text)
...
async def enhance_transcript(request: EnhancementRequest) -> EnhancementResponse:
    ...
    try:
        ...
        # 调用 DeepSeek 优化
        enhanced_turns = await deepseek_client.enhance_transcript(transcript_turns)
